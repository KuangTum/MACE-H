...
class ScaleShiftMACE(MACE):
    def __init__(
        ...
        **kwargs,
        ...
        self.scale_shift = ScaleShiftBlock(
            scale=1, shift=atomic_inter_shift
        )
    ...
    def forward(
        ...
        edge_attrs = self.spherical_harmonics(vectors)
        edge_feats = self.radial_embedding(lengths)
        # print("edge_feats", to_dense_adj(edge_index=data["edge_index"], batch=data["batch"], edge_attr=edge_feats[:,0])[:,10,:])
        # Interactions
        node_es_list = []
        for interaction, product, readout in zip(
            self.interactions, self.products, self.readouts
        ...
        print("node_inter_es", node_inter_es)
...
class MFN_MACE(torch.nn.Module):
    def __init__(
        self,
        r_max: float,
        num_bessel: int,
        ...
        use_matrix_all: Optional[bool] = False,
        ...
        self.use_matrix_all = use_matrix_all
        ...
        if num_interactions == 1 or self.use_matrix_all:
            ...
            self.bond_interactions.append(None)
        ...
        for i in range(num_interactions - 1):
            ...
            if i == num_interactions - 2:
                ...
                self.readouts.append(LinearReadoutBlock(hidden_irreps))
            if i == num_interactions - 2 or self.use_matrix_all:
                bond_inter = MatrixFunctionBlock(
    ...
    def forward(
        ...
        for interaction, product, readout, bond_interaction in zip(
            ...
            if bond_interaction is not None:
...
class ScaleShiftMFN_MACE(MFN_MACE):
    def __init__(
        self,
        atomic_inter_scale: float,
        atomic_inter_shift: float,
        **kwargs,
        ...
        super().__init__(**kwargs)
        self.scale_shift = ScaleShiftBlock(
            scale=1.0, shift=atomic_inter_shift
        )
